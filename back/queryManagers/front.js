...
const defaultFileIfFolder = "homepages/homepage.html";
...
function manageRequest(request, response) {
    ...
    let parsedUrl = url.parse(baseFrontPath + request.url);
    ...
    fs.exists(pathName, async function (exist) {
        if (!exist){
            console.log(`The file doesn't exist: ${pathName}`);
            send404(pathName, response);
            return;
        }
        console.log(`The file exist: ${pathName}`);
